...
const solutions = {
  "construction-os": {
    ...
    context: "Construction companies lose margin when commercial, site and finance information lives in separate spreadsheets and messages. OPSYNQ creates one operational record from the first enquiry through estimating, delivery, variations, invoicing and final handover.",
  ...
  "manufacturing-os": {
    ...
    context: "Manufacturing teams need a reliable flow from customer demand to materials, capacity, production, quality and fulfilment. OPSYNQ replaces fragmented tracking with a custom operational model that reflects the real stages, exceptions and controls of the factory.",
  ...
  "distribution-os": {
    ...
    context: "Distribution businesses need customer terms, supplier purchasing, inventory, fulfilment and margin data to move together. OPSYNQ creates a tailored commercial and operational platform without forcing complex pricing and order logic into spreadsheets.",
  ...
  "logistics-os": {
    ...
    context: "Logistics operations depend on rapid handoffs between customers, dispatch, drivers, documentation and finance. OPSYNQ turns those handoffs into visible workflows with clear exceptions, automated communication and management dashboards.",
  ...
  "property-management-os": {
    ...
    context: "Property operations create a constant flow of tenant communication, contracts, supplier work, maintenance requests and financial events. OPSYNQ connects the portfolio, people, documents and workflows so management sees issues before they become reporting problems.",
...
export function generateStaticParams() {
  return Object.keys(solutions).filter((slug) => slug !== "construction-os").map((slug) => ({ slug }));
}
...
export default async function SolutionPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const schema = {
    ...
    "@graph": [
      { "@type": "Service", "@id": `${SITE_URL}/solutions/${slug}#service`, name: solution.name, description: solution.intro, serviceType: [solution.keyword, ...solution.secondary], provider: { "@type": "Organization", "@id": `${SITE_URL}/#organization`, name: "OPSYNQ", url: SITE_URL }, areaServed: ["Netherlands", "Norway", "Sweden", "Denmark"] },
      { "@type": "BreadcrumbList", itemListElement: [{ "@type": "ListItem", position: 1, name: "Home", item: SITE_URL }, { "@type": "ListItem", position: 2, name: "Industry solutions", item: `${SITE_URL}/solutions` }, { "@type": "ListItem", position: 3, name: solution.name, item: `${SITE_URL}/solutions/${slug}` }] },
    ],
  };
  ...
  return (
    <main className="locale-page service-page">
      ...
      <section className="locale-hero shell service-hero"><div className="eyebrow"><i /> INDUSTRY BUSINESS OPERATING SYSTEM</div><h1>{solution.name}.<br /><em>Your operation, connected.</em></h1><p>{solution.intro} The same proven operating core is configured around the way your company actually works.</p><div className="actions"><Link className="button primary" href="/contact">Discuss your workflow <span>↗</span></Link><Link className="button ghost" href="/solutions">All industries <span>↓</span></Link></div><div className="locale-orbit" aria-hidden="true"><b>CRM</b><b>OPS</b><b>DATA</b><b>AI</b><i>{solution.name.split(" ")[0].toUpperCase()}<br />OS</i></div></section>
      ...
      <section className="market-industries"><div className="shell market-industries-grid"><div><div className="kicker">RELATED BUSINESS SOFTWARE</div><h2>Connect the industry layer to the whole company.</h2></div><div><p>{solution.name} works with the same CRM, ERP, workflow and AI capabilities used across OPSYNQ. This gives sales, operations, employees, finance and management one connected view.</p><div className="text-link-grid">{serviceLinks.slice(0, 6).map((service) => <Link href={service.href} key={service.href}>{service.label} ↗</Link>)}</div></div></div></section>
